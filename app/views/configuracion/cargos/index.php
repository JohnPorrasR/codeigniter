...
<script>
    $("#n_id_cargo_entidad").attr('readonly', 'readonly');
    $("#n_id_cargo_entidad").val(0);
    ...
    $.ajax({
        ...
        success : function(response){
            for (var i = 0; i < response.cargos.length; i++) {
                $("#m_cargo_id").append('<option value="'+response.cargos[i]['n_id_cargo']+'">'+response.cargos[i]['x_cargo_desc']+'</option>');
            }
            $("#m_nodo").html('<option value="0">Seleccione</option>');
            for (var i = 0; i < response.cargos.length; i++) {
                $("#m_nodo").append('<option value="'+response.cargos[i]['n_id_cargo']+'">'+response.cargos[i]['x_cargo_desc']+'</option>');
            }
...
<script>
    ...
    $(document).ready(function(){
        $("#"+$("#txtLabelId").val()).attr("readonly","readonly");
        ...
        tabla1 = $('#tabla1').DataTable({
            ...
            "ajax": {
                "url" : $("#txtBaseUrl").val()+'/'+$("#txtUrl").val()+"/mostrar_cargos",
                "type": "POST"
            },
            "columns": [
                {
            ...
            if(cod > 0)
            {
                ...
                if(tab == 0)
                {
                    $.ajax({
                        ...
                        processData: false,
                        data: data,
                        url: $("#txtBaseUrl").val()+$("#txtUrl").val()+'/obtener_cargo',
                        // async: true,
                        success : function(response){
                            $("#"+$("#txtLabelId").val()).val(response.data[0]["n_id_cargo"]);
                            $("#x_cargo_desc").val(response.data[0]["x_cargo_desc"]);
                ...
                else
                {
                    ...
                    $.ajax({
                        ...
                        // async: true,
                        success : function(response){
                            $("#n_id_cargo_entidad").val(response.data[0]["n_id_cargo_entidad"]);
                            $("#m_cargo_id").val(response.data[0]["m_cargo_id"]);
                            $("#m_entidad_id").val(response.data[0]["m_entidad_id"]);
                            $("#m_nodo").val(response.data[0]["m_nodo"]);
                        }
                    });
                }
